...
class PredicateClassifier(nn.Module):
    ...
    # def __init__(self, n_relations, embedding_dim, hop, dropout, unk_mask):
    ...
    def forward(self, predicates):
        ...
        :param predicates:  (bsz, n_relations). values = pred_ids.
        :return:
        '''

        # predicates = self.dropout(predicates)

        pred_size = predicates.size()  # (bsz, n_relations)

        u = [self.init_hidden(pred_size[0])]
        for hop in range(self.max_hops):
            m_A = self.C[hop](predicates.long())  # (bsz, n_relations, hidden_size).

            u_temp = u[-1].unsqueeze(1).expand_as(m_A)  # (bsz, n_relations, hidden_size)
            prob = self.softmax(torch.sum(m_A * u_temp, 2))  # (bsz, n_relations). eq (1).

            m_C = self.C[hop + 1](predicates.long())  # (bsz, n_relations, hidden_size).
